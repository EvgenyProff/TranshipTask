...
string out_path, log_path;														//путь для вывода решения и логов
...
void output(bool flag_final) {
	ofstream final(out_path), log(log_path);
	for (ll i = 1; i <= m; ++i) {
		if (flag_final) {
			final << endl;
			log << endl;
		...
		for (ll j = 1; j <= n; ++j) {
			if (flag_final) {
				final << shipment[i][j] << '*' << prices[i][j] << '\t';
				log << shipment[i][j] << '*' << prices[i][j] << '\t';
			}
			else {
				log << shipment[i][j] << '*' << prices[i][j] << '\t';
			}
...
bool find_zero_cell_to_insert_in_basis() {
	...
	while (1) {
		...
		basis_cells[i][j] = 1;
		fout << "В базис введена ячейка " << i << " " << j << endl;
...
void p_method() {
	...
	while (1) {
		...
		for (ll i = 1; i <= m; ++i) {
			...
			fout << "Стоимость перевозки: " << full_price_of_shipment();
		}
	}
}
...
void solve(char path[]) {
	...
	out_path = path;
	log_path = path;
	if (path[sizeof(path) - 4] == '.') {
		out_path.insert(sizeof(path) - 7, "-Решение");
	}
	else {
		log_path.insert(sizeof(path) - 8, "-Логи");
	}
	ll frequest = 0;
	for (ll j = 1; j <= n; ++j) {
		frequest += request[j];
	...
	ofstream fout(log_path);
	if (foffer > frequest) {
		fout << "Предложение больше спроса на " << foffer - frequest << " единиц груза. Создайте фиктивного потребителя.\n";
	}
	else {
		if (foffer < frequest) {
			fout << "Предложение меньше спроса на " << frequest - foffer << " единиц груза. Создайте фиктивного поставщика.\n";
		}
	}
	...
	nc_method();
	ll now = clock();
	fout << endl << "Для вычисления понадобилось (сек): " << ((ld)(now - tick)) / CLOCKS_PER_SEC << endl;
	fout << "Стоимость перевозки методом северо-западного угла: " << full_price_of_shipment();
	output(false);
	to_zero();
	fout << endl;
	tick = clock();
	...
	now = clock();
	fout << endl << "Для вычисления понадобилось (сек): " << ((ld)(now - tick)) / CLOCKS_PER_SEC << endl;
	fout << "Стоимость перевозки методом минимальных тарифов: " << full_price_of_shipment();
	output(false);
	to_zero();
	fout << endl;
	tick = clock();
	...
	now = clock();
	fout << endl << "Для вычисления понадобилось (сек): " << ((ld)(now - tick)) / CLOCKS_PER_SEC << endl;
	fout << "Стоимость перевозки методом Фогеля: " << full_price_of_shipment();
	output(false);
	fout << endl;
	tick = clock();
	p_method();
